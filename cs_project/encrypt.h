...
std::vector<unsigned long> Encryption::padding(std::vector<unsigned long> vector)
{
	...
	for (int i = 0; i < k / 8; i++)
	{
		vector.push_back(0x00000000);
	}
	std::bitset <64> bits_64(length);
	std::string blob_string = bits_64.to_string();

	std::bitset<8> temp(blob_string.substr(0, 8));
	vector.push_back(temp.to_ulong());

	for (int i = 0; i < 63; i = i + 8)
	{
		std::bitset<8> temp2(blob_string.substr(i, 8));
